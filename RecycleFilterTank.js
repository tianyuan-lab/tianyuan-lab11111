...
class RecycleFilterTank {
    ...
    createPipeSystem() {
        ...
        // 循环管道
        const recyclePipe = this.createPipeSegment(0.35, 10, { x: radius + 5, y: height * 0.3, z: 0 });
        recyclePipe.rotation.z = Math.PI / 2;
        pipeGroup.add(recyclePipe);
    ...
    createRailings(parent) {
        ...
        for (let i = 0; i < 16; i++) {
            ...
            if (i < 15) {
                ...
                const nextX = Math.cos(nextAngle) * (radius + 2);
                const nextZ = Math.sin(nextAngle) * (radius + 2);
                
                const railLength = Math.sqrt(Math.pow(nextX - x, 2) + Math.pow(nextZ - z, 2));
                const railGeometry = new THREE.CylinderGeometry(0.03, 0.03, railLength, 8);
                const railMesh = new THREE.Mesh(railGeometry, this.materials.railing);
                
                railMesh.position.set((x + nextX) / 2, height + 1.5, (z + nextZ) / 2);
                railMesh.lookAt(nextX, height + 1.5, nextZ);
    ...
    createValve(size, position) {
        ...
        // 手轮辐条
        for (let i = 0; i < 6; i++) {
            const spokeGeometry = new THREE.BoxGeometry(size * 0.1, 0.05, size * 0.4);
            const spokeMesh = new THREE.Mesh(spokeGeometry, this.materials.steel);
            spokeMesh.position.y = size * 0.85;
            spokeMesh.rotation.y = (i / 6) * Math.PI * 2;
    ...
    createSafetyEquipment() {
        ...
        // 消防栓
        const hydrantGeometry = new THREE.CylinderGeometry(0.2, 0.2, 1, 8);
        const hydrantMaterial = new THREE.MeshPhongMaterial({ color: 0xFF0000 });
        const hydrantMesh = new THREE.Mesh(hydrantGeometry, hydrantMaterial);
        hydrantMesh.position.set(radius + 4, 0.5, 3);
        hydrantMesh.castShadow = true;
        safetyGroup.add(hydrantMesh);
    ...
    createLabelsAndSigns() {
        ...
        // 主标签 - 回收水箱
        const mainLabel = this.createTankLabel('回收水箱', '#2E86AB');
    ...
    createLiquidContent() {
        ...
        const liquidHeight = this.tankConfig.height * 0.7; // 70%液位
    ...
    /** 获取水箱中部世界坐标（用于外部管道连接） */
    getMiddleWorldPosition() {
        const local = new THREE.Vector3(0, this.tankConfig.height / 2, 0);
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = RecycleFilterTank;
} else if (typeof window !== 'undefined') {
    window.RecycleFilterTank = RecycleFilterTank;
}